...
Sample_var_with_question_mark should be around 1.
The reason is that for each Monte Carlo sample, since $\theta = 10^{10}+\epsilon$ where $\epsilon \sim N(0,1)$, when taking $\theta^2$, due to the limit of storage, the floating point numbers can not be stored fully and will lead to bias.
However, when we decrease the level of $\theta$ as $10^{6}+\epsilon$ or $10^{7}+\epsilon$, Sample_var_with_question_mark is more around 1.
...
**Answer:** as the $\Delta x$ decreases, the relative error is decreasing.
the slope is around 2.

```{r, eval=FALSE}
#' Approximate derivative via centered-difference approximation
approximate_deriv <- function(func, x, dx) {
  fx_delta_x = func(x+dx)
  fx_delta_x_minus = func(x-dx)
  value = (fx_delta_x-fx_delta_x_minus)/2/dx
  return(value)
}

log_logistic_func <- function(x) {
  value = log(exp(x)/(1+exp(x)))
  return(value)
}
...
**Answer:** No, the relative error plot shows as the $\Delta x$ decreases, relative error firstly decreases but at $\Delta x$ is around $2^{-18}$, the relative error is increasing with "jigsaw" trend.
...
**Answer**: the slope is close to -1 in the "latter part" of your plotted line.
Before the changing point, relative error is dominated by $O(\Delta x^2)/f'(x)$, the slope will be $\frac{\log_2(O(\Delta x_1^2)/O(\Delta x_2^2))}{\log_2(\Delta x_1/ \Delta x_2)} = 2$, relative error is dominated by $(2^{-52}/2\Delta x) f'(x)$, the slope will be $\frac{\log_2((2^{-52}/\Delta x_2)/(2^{-52}/\Delta x_1))}{\log_2(\Delta x_1/\Delta x_2)} = \frac{\log_2(\Delta x_2/\Delta x_1)}{\log_2(\Delta x_1/\Delta x_2)} =-1$.
