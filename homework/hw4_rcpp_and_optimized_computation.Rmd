...
Most of the time I got 1022, and sometimes I got 1023, and sometimes I got Inf. 

In C++ language, the initial index begins from 0, from 0 to 1022, there are number of 1023 numbers. But from index 1, there is no 1024 element in the vector. 
...
It return two columns, the first column is always a vector of length 4, each element is 1024. Sometimes it will return NA or very large values in the second column or 512. Because C++ doesn't check the dimension of the input, whether the matrix inputs are compatible or not, it will still do the calculation and give the output. 
...
(1) Running the first code chunk, the R session will be aborted. It is due to the length of X is longer than y, so to call y[i] will be out of bound if i is larger than the size of y.

(2) Running the second code chunk, since the length of X is shorter than y, so only the size of x will be called, will not be out of bound.
...
row_cal = row_col_dot_matmat(A, B)
diff = as.vector(row_cal - A %*% B)
summary(diff)
```
...
**Answer**: By comparing the two results, using row oriented is slower (27s), but column oriented is faster (183 ms). This is because how the c++ stores the matrix as the row array. But R store the arrays as the column direction. 

For the second question, comparing with R function via `%*%` in R, col oriented is still faster. 


```{r}
# Check and benchmark
col_cal = col_oriented_matmat(A, B)
diff_col = as.vector(col_cal - A %*% B)
summary(diff_col)
...
**Answer**: Using cluster, if we shut down all the optimization compilers, it takes longest time across all the algorithms. There is slightly difference using different optimization compilers for col_directed matrix calculation, row_directed matrix calculation and using R `%*%`. But for dgemm_eigen(), if we open AVX, the time decreases a lot. 

Across all the algorithms, row_directed matrix calculation is still lowest. 

```{r}
output_option = c("none","AVX","SSE","AVX and FMA","SSE, AVX and FMA")
output_dgemm_time = c("170 ms","72 ms","148 ms","58 ms","55 ms")
output_row_time = c("24 s","23.5 s","23.6 s","23.8 s","23.7 s")
output_col_time = c("946 ms","942 ms","957 ms","941 ms","975 ms")
output_R_fun_time = c("739 ms","733 ms","702 ms","703 ms","721 ms")

data.frame(output_option = output_option,
           dgemm = output_dgemm_time,
           row = output_row_time,
           col = output_col_time,
           R_fun = output_R_fun_time)

```


```{r, warning=FALSE, eval=T, message=FALSE, include=FALSE}
